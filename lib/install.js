...
async function install (where, args, cb) {
  ...
  try {
    ...
    const addedPackages = diffTopDeps(initialTree, tree)
    const stop = Date.now()
    /**
     * TODO:
     * - Add audit
    ...
    printResults({ start, stop }, addedPackages, tree)
    cb()
  } catch (err) {
...
function printResults (timing, addedPackages, tree) {
  ...
  output('')
  for (const [dep, version] of Object.entries(addedPackages)) {
    output(`+ ${dep}@${version}`)
  }

  // Collect information about command action taken
  const pkgCount = Object.keys(addedPackages).length
